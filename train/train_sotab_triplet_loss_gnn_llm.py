...
# This reduces fragmentation by allowing segments to grow.
...
def train():
    ...
    # load language model
    if args.shortcut_name in [ 'Qwen/Qwen3-Embedding-0.6B', 'Qwen/Qwen3-Embedding-4B', 'Qwen/Qwen3-Embedding-8B']:

        from transformers.models.qwen3.configuration_qwen3 import Qwen3Config
        from layers.llm_task_layer import Qwen3ForMatching

        if args.from_scratch:
            # No pre-trained checkpoint
            model_config = Qwen3Config.from_pretrained(shortcut_name)
            model = Qwen3ForMatching(model_config)
        else:
        ...
        raise ValueError("Could not find / load the language model: {}".format(args.shortcut_name))

    model.init_by_task(task_num_class_dict, args.emb_dim)
    device = model.model.embed_tokens.weight.device
    model.projections.to(device)
    model.norms.to(device)

    # LORA settings and Config
    if args.lora_enabled:
        ...
        lora_config = LoraConfig(
            ...
            bias="none",
            init_lora_weights=args.init_lora_weights,  # "olora"
        ...
        optimizer_grouped_parameters = [
            ...
            },
            {
                "params": [
                    ...
                    if not any(nd in n for nd in no_decay) and ('projections' not in n) and task not in n
                ],
                "weight_decay":
                    0.1,
                "lr": args.lr
            },
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if any(nd in n for nd in no_decay) and ('projections' not in n) and task not in n
                ],
                "weight_decay":
                    0.0,
                "lr": args.lr
            },
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if not any(nd in n for nd in no_decay) and ('projections' in n) and task in n
                ],
                "weight_decay":
                0.001,
                "lr": args.lr
            },
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if any(nd in n for nd in no_decay) and ('projections' in n) and task in n
                ],
                "weight_decay":
                    0.0,
                "lr": args.lr
        ...
        for k, (task, train_dataset, valid_dataset, train_dataloader,
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                with autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    # TODO
                    eos_indexes = batch['cls_idx']
                    # filtered_logits = torch.zeros(eos_indexes.shape[0], logits.shape[2]).to(device=device, dtype=torch.bfloat16)
                    filtered_logits = torch.zeros(eos_indexes.shape[0], logits.shape[2]).to(device=device)

                    assert eos_indexes.shape[0] == batch["label"].shape[0]

                    for n in range(eos_indexes.shape[0]):
                    ...
                    if 'CTA' in task:
                        ...
                        raise ValueError("Could not find the right task type.")
            ...
            for batch_idx, batch in enumerate(valid_dataloader):
                ...
                with autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    if 'CTA' in task:
                        ...
                        raise ValueError(" Could not load the right taks type... ")
            ...
            if vl_micro_f1 > best_vl_micro_f1s[k]:
                ...
                model.save_pretrained(model_save_path + "/qwen_st2")
