...
def train():
    ...
    for epoch in range(num_train_epochs):
        for k, (task, train_dataset, valid_dataset, train_dataloader,
            ...
            # train_dataset.data_list = train_dataset.data_list[:1800]
            ...
            # gittab (train -> 100%, validation, test -> 100%)
            # validation set does not cover all the labels 100% in vocabulary

            # Step 1: Get the set of labels present in the true and predicted lists
            present_labels = set(vl_true_list) | set(vl_pred_list)

            # Step 2: Get the set of target labels (from the target_names)
            target_labels = set([item[0] for item in sorted(
                [(key, value) for key, value in data_dict[task]['idx2label'].items()],
                key=lambda x: int(x[0]))])

            # Step 3: Find the missing labels
            missing_labels = target_labels - present_labels
            print(f"Missing Labels: {[ data_dict[task]['idx2label'][idx] for idx in missing_labels]}")
            missing_labels =  set([data_dict[task]['idx2label'][idx] for idx in missing_labels])

            target_names = [item[1] for item in sorted(
                [(key, value) for key, value in data_dict[task]['idx2label'].items() if value not in missing_labels],
                key=lambda x: int(x[0]))]
